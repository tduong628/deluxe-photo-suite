                 ...
                 <div className="relative">
                     ...
                     <label className="flex flex-col items-center justify-center w-full aspect-[4/3] rounded-[2rem] border-2 border-dashed border-gray-300 bg-white hover:bg-gray-50 transition-colors cursor-pointer group ios-btn-press">
                        <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                             <Icon name="upload" className="w-8 h-8 text-gray-400 group-hover:text-rose-500 transition-colors" />
                        </div>
                    ...
                    <div className="relative w-full aspect-square bg-gray-100 rounded-[2rem] overflow-hidden shadow-[0_20px_40px_-12px_rgba(0,0,0,0.15)] border border-white/50">
                        {isLoading ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-20">
                                <Loader />
                                <p className="mt-4 text-sm font-semibold text-rose-600 animate-pulse px-6 text-center">{loadingStage}</p>
                            </div>
                        ) : null}
                        ...
                        {selectedBrandedImage && !isLoading && (
                            <button 
                                ...
                                className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-lg text-rose-600 ios-btn-press z-20 group"
                    ...
                    {brandedImages && !isLoading && (
                        <div className="flex gap-2 mt-4 overflow-x-auto no-scrollbar py-2">
                             {brandedImages.map((image, index) => (
                                <button key={index} onClick={() => setSelectedBrandedImage(image)} className={`flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${selectedBrandedImage === image ? 'border-rose-500 scale-105 shadow-md' : 'border-transparent opacity-70'}`}>
                            ...
                            >
                                {categories.map(cat => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap ios-btn-press ${activeCategory === cat.id ? 'bg-rose-500 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                    >
                        ...
                        <div key={activeCategory} className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {templatesByCategory[activeCategory].map((template, index) => (
                                <button 
                                    ...
                                    ${activeTemplateId === template.id ? 'border-rose-500 ring-2 ring-rose-100' : 'border-gray-100'}`}
                                >
